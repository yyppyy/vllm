...
TORCH_LIBRARY_EXPAND(TORCH_EXTENSION_NAME, m) {
  ...
  // Python bindings (all indices/counts Long)
  ...
  m.impl("eplb_route_greedy", torch::kCUDA,
    [](const at::Tensor& off, const at::Tensor& idx, int64_t P) {
        auto n = (int64_t)off.size(0) - 1;
        auto chosen = at::empty({n}, off.options().dtype(at::kLong));
        greedy_smallest_choice_first_cuda(off, idx, chosen, (int64_t)P);
        return chosen; // long
    });
    ...
    m.impl("eplb_route_exact", torch::kCUDA,
        [](const at::Tensor& off, const at::Tensor& idx, int64_t P) {
            auto n = (int64_t)off.size(0) - 1;
            auto chosen = at::empty({n}, off.options().dtype(at::kLong));
            auto L_tensor = exact_min_max_activations_cuda(off, idx, chosen, (int64_t)P); // long scalar
    ...
    m.impl("eplb_select_replica", torch::kCUDA,
        [](const at::Tensor& l2p, const at::Tensor& lrc, const at::Tensor& active,
            ...
            select_replica_on_rank_cuda(l2p, lrc, active, chosen_rank, chosen_rep, (int64_t)P);
            return chosen_rep; // long
        });
