...
@register_fake("_C_dispatch_combine::dar_phase_a")
def _dar_phase_a_fake(
    input: torch.Tensor,
    topk_ids: torch.Tensor,
    topk_weights: torch.Tensor,
    config_tensor: torch.Tensor,
...
@register_fake(
    "_C_dispatch_combine::dar_push_and_barrier")
def _dar_push_and_barrier_fake(
    config_tensor: torch.Tensor,
) -> None:
    return


@register_fake("_C_dispatch_combine::dar_phase_c")
def _dar_phase_c_fake(
    config_tensor: torch.Tensor,
    num_logical_experts: int,
    world_size: int,
) -> None:
...
@register_fake("_C_dispatch_combine::dar_phase_d1")
def _dar_phase_d1_fake(
    expert_num_tokens: torch.Tensor,
    num_physical_experts: int,
) -> None:
    return
...
@register_fake("_C_dispatch_combine::dar_phase_d2")
def _dar_phase_d2_fake(
    expert_topk_ids: torch.Tensor,
    expert_topk_weights: torch.Tensor,
    expert_num_tokens: torch.Tensor,
    data_remap: torch.Tensor,
...
@register_fake("_C_dispatch_combine::dar_phase_e")
def _dar_phase_e_fake(
    config_tensor: torch.Tensor,
) -> None:
    return
