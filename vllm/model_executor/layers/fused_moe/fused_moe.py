...
# stamps boundaries between the 5 kernels (align, gemm_gu, silu, quant,
# gemm_dn) using a tiny CUDA op that reads %globaltimer; the stamps are
    ...
    def apply(
        ...
        if _explat_on:
            if not hasattr(self, '_explat_stamps'):
                self._explat_stamps = (
                    explat_runtime.alloc_layer_stamps())
            _explat_stamps = self._explat_stamps
            torch.ops._C_explat.record_stamp(_explat_stamps, 0)
        ...
        sorted_token_ids, expert_ids, num_tokens_post_padded = (
            moe_align_block_size(topk_ids, config['BLOCK_SIZE_M'],
                                 global_num_experts, expert_map))
        if _explat_on:
            torch.ops._C_explat.record_stamp(_explat_stamps, 1)
        _e1 = _stage_evt()
        ...
        if _explat_on:
            torch.ops._C_explat.record_stamp(_explat_stamps, 2)
        ...
        if (expert_tokens_meta is not None
            ...
            self.activation(
                ...
                intermediate_cache1.view(-1, N))
        if _explat_on:
            torch.ops._C_explat.record_stamp(_explat_stamps, 3)
        ...
        if _explat_on:
            torch.ops._C_explat.record_stamp(_explat_stamps, 4)
        ...
        if _explat_on:
            torch.ops._C_explat.record_stamp(_explat_stamps, 5)
            # Stash for the prepare_finalize logger op to consume.
            pf = getattr(self, '_explat_log_target', None)
            if pf is not None:
                pf._explat_stamps = _explat_stamps
