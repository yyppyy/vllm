...
class FusedMoEModularKernel(torch.nn.Module):
    ...
    def forward(
        ...
        # Breakdown profile: gate_end stamp goes here (just before the
        ...
        # Breakdown profile: dispatch_end stamp (= expert_start). All
        # dispatch / dar_compact / barrier kernels have been launched
        # by now; this stamp closes the dispatch region.
        if _bd_on:
            torch.ops._C_explat.record_stamp(_bd_py_stamps, 4)
        ...
        if a1q.numel() == 0:
            ...
            # ExpLat profile: TritonExperts.apply stashes its
            # per-layer stamp tensor on self.prepare_finalize via this
            # back-pointer so the captured logger op can read it.
            if hasattr(self.prepare_finalize, '_moe_layer_idx'):
                self.fused_experts._explat_log_target = (
                    self.prepare_finalize)
