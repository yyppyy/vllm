...
#     side-by-side without colliding; this script enables only the
#     breakdown one.
...
#     attention_ns=X gating_ns=Y routing_ns=Z dispatch_ns=A
#     expert_ns=B combine_ns=C
...
# OFF so the three profilers don't dual-fire.
unset VLLM_EXP_LATENCY_PROFILE
unset VLLM_EXP_LATENCY_READY_FILE
unset VLLM_EXP_LATENCY_LOG_PATH
unset VLLM_DC_PROFILE
READY_FILE="$RES_DIR/$RUN_HASH/breakdown_ready"
BREAKDOWN_LOG="$RES_DIR/$RUN_HASH/server_breakdown.log"
rm -f "$READY_FILE" "$BREAKDOWN_LOG"
export VLLM_BREAKDOWN_PROFILE=1
export VLLM_BREAKDOWN_READY_FILE="$READY_FILE"
export VLLM_BREAKDOWN_LOG_PATH="$BREAKDOWN_LOG"
...
# after the last replay. Give it 35s of slack before tearing the
# server down.
echo "=== Bench done; sleeping 35s to let the breakdown poller drain ==="
sleep 35

if [[ -s "$BREAKDOWN_LOG" ]]; then
  echo "=== Breakdown log written: $BREAKDOWN_LOG ($(wc -l <"$BREAKDOWN_LOG") lines) ==="
else
  echo "WARN: Breakdown log $BREAKDOWN_LOG missing or empty"
fi
